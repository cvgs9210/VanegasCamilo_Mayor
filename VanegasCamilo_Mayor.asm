...
find_max:
    ...
    blt $t4, $t3, next_number      # Si $t4 es menor que $t3, omitir actualización
    move $t3, $t4      # Si $t4 es mayor, actualizar el valor de $t3 (nuevo mayor)

next_number:
    addi $t1, $t1, 4    # Moverse al siguiente número en el array
    bne $t1, $zero, find_max   # Repetir hasta que se lean todos los números

print_result:
    # Imprimir el mensaje "El número mayor es: "
    li $v0, 4             # Cargar el código del servicio 4 en $v0 (imprimir cadena)
    la $a0, result        # Cargar la dirección de la cadena "result" en $a0
    syscall               # Llamar a la syscall para imprimir la cadena
    
    # Imprimir el número mayor (almacenado en $t3)
    li $v0, 1             # Cargar el código del servicio 1 en $v0 (imprimir entero)
    move $a0, $t3         # Mover el valor de $t3 (número mayor) a $a0
    syscall               # Llamar a la syscall para imprimir el entero
    
    # Salir del programa
    li $v0, 10            # Cargar el código del servicio 10 en $v0 (salir del programa)
